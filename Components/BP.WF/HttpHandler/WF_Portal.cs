    ...
    public class WF_Portal : DirectoryPageBase
    {
        ...
        public string Flows_InitSort()
        {
            ...
            if(SystemConfig.CCBPMRunModel == CCBPMRunModel.Single)
            {
                ...
                if (gloVar.RetrieveFromDBSources() != 0)
                {
                    sql = "SELECT No as \"No\",Name as \"Name\", 0 as WFSta2, 0 as WFSta3, 0 as WFSta5 FROM WF_FlowSort WHERE  No='"+WebUser.FK_Dept+"' OR ParentNo='"+ WebUser.FK_Dept+"' ORDER BY Idx ";
