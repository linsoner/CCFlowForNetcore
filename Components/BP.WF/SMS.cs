    ...
    public class SMS : EntityMyPK
    {
        ...
        public static bool SendEmailNowAsync(string mail, string mailTitle, string mailDoc)
        {
            try
            {
                ...
                //http://127.0.0.1:9200/Portal/Standard/Login.htm
                //HostURL
                string hostUrl = BP.Difference.SystemConfig.GetValByKey("HostURL", "http://192.168.0.253:9100/");
                mailDoc = DataType.ParseText2Html(mailDoc);
                if(mailDoc.IndexOf("打开") >0)
                {
                    //mailDoc = mailDoc.Substring(0, mailDoc.IndexOf("打开")) + "打开" + hostUrl  + "Portal/Standard/Login.htm";
                    mailDoc = mailDoc.Substring(0, mailDoc.IndexOf("打开")) + "<a href='"+ hostUrl + "Portal/Standard/Login.htm"+"'>登录流程系统</a>";
                    //<a href='http://www.example.com'>验证链接</a>
                }

                string displayName =  BP.Difference.SystemConfig.GetValByKey("SendEmailDisplayName", "LCRBPM");
                ...
                messageToSend.From.Add(new MailboxAddress(emailAddr, emailAddr));
                messageToSend.Body = new TextPart(TextFormat.Html) { Text = mailDoc };
                messageToSend.To.Add(new MailboxAddress(mail, mail));
                messageToSend.Subject = mailTitle;

                using (var client1 = new MailKit.Net.Smtp.SmtpClient())
                {
                    client1.Connect(client.Host, client.Port, client.EnableSsl);

                    // Note: only needed if the SMTP server requires authentication
                    client1.Authenticate(emailAddr, emailPassword);

                    var ss = client1.Send(messageToSend);
